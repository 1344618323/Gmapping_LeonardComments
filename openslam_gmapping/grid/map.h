...
class Map{
	public:
		...
		inline Cell& cell(const Point& p);//先从worldSize转成mapSize
...
Map<Cell,Storage,isClass>::Map(const Point& center, double worldSizeX, double worldSizeY, double delta):
	m_storage((int)ceil(worldSizeX/delta), (int)ceil(worldSizeY/delta)){//啥都没做的初始化，为4000,4000，指向一个125*125（patchMagnitude默认是5）二维指针数组Array2D<autoptr< Array2D<Cell> > >，每个指针指向32*32的Cell数组Array2D<Cell>
	m_center=center;					//啥都没做的初始化，为（0，0）,worldSize的中心
	m_worldSizeX=worldSizeX;	//啥都没做的初始化，为200，单位m?（7.24）
	m_worldSizeY=worldSizeY;	//啥都没做的初始化，为200
	m_delta=delta;						//啥都没做的初始化，为0.05
	m_mapSizeX=m_storage.getXSize()<<m_storage.getPatchSize();	//啥都没做的初始化，为4000>>5<<5
	m_mapSizeY=m_storage.getYSize()<<m_storage.getPatchSize();
	m_sizeX2=m_mapSizeX>>1;		//啥都没做的初始化，为4000>>1=2000
	m_sizeY2=m_mapSizeY>>1;
}
...
DoubleArray2D* Map<Cell,Storage,isClass>::toDoubleArray() const{
	...
	for(int x=0; x<getMapSizeX()-1; x++)
		for(int y=0; y<getMapSizeY()-1; y++){
			IntPoint p(x,y);
			darr->cell(p)=cell(p);//Array2D<Cell>的cell
		}
	return darr;
}
