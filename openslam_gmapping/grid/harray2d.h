...
//继承一个类，这个类是一个存放智能指针的二维数组，这些智能指针指向一个存放Cell的二维数组
template <class Cell>
class HierarchicalArray2D: public Array2D<autoptr< Array2D<Cell> > >{
	public:
		...
		//索引时，x，y都要除以patchMagnitude
		inline const Cell& cell(int x, int y) const;
		inline Cell& cell(int x, int y);
		inline bool isAllocated(int x, int y) const;
		inline AccessibilityState cellState(int x, int y) const ;//指针指向的放Cell二维数组Array2D<Cell>是否申请了Cell空间，如果有2，如果没有但在范围内1，不在范围内0
		inline IntPoint patchIndexes(int x, int y) const;
		
		inline const Cell& cell(const IntPoint& p) const { return cell(p.x,p.y); }
		inline Cell& cell(const IntPoint& p) { return cell(p.x,p.y); }//返回小cell（二维指针的二维指针指向的实例），若Cell二维数组Array2D<Cell>未申请空间，则申请空间在返回对应的小cell
	...
	protected:
		...
		int m_patchMagnitude;//斑块大小 m_xsize=xsize>>patchMagnitude, m_ysize=ysize>>patchMagnitude
		int m_patchSize;//1<<m_patchMagnitude
};
...
Cell& HierarchicalArray2D<Cell>::cell(int x, int y){
	...
	autoptr< Array2D<Cell> >& ptr=this->m_cells[c.x][c.y];
	return (*ptr).cell(IntPoint(x-(c.x<<m_patchMagnitude),y-(c.y<<m_patchMagnitude)));//返回(1<<m_patchMagnitude)*(1<<m_patchMagnitude)Cell数组中对应的cell
}
