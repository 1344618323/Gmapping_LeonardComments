...
template<class Cell, const bool debug=false> class Array2D{
	public:
		...
		inline bool isInside(int x, int y) const; //x，y是否能在index中找到
		inline const Cell& cell(int x, int y) const; //返回x，y处的cell
		inline Cell& cell(int x, int y);
		// cell的state：在索引里（Inside|Allocated) or 不在索引里Outside
